...
/// 往復検証は Cloudflare Worker (`watashiba.kagayoi.com`) 相手の手動 e2e で担保する
...
public class WebSocketRelayTransportTests
{
    ...
    public void コンストラクタ引数のrelayUrlとpairIdとroleが受け入れられる()
    {
        ...
        var transport = new WebSocketRelayTransport("wss://watashiba.kagayoi.com/ferry-relay", "abc_def", "answer");
        Assert.Equal(ConnectionRoute.Relay, transport.Route);
        Assert.False(transport.IsConnected);
    }
}
