...
- ペアリングコードを使い捨て式にし、端末 ID だけを知る第三者による宛先登録とコードの再利用を防止しました
- 受信ファイルの SHA-256 値を保存処理の前に検証し、壊れたメタデータを安全に拒否するようにしました

### 変更

- カメラを使わないペアリングのコード形式を更新しました。旧版とペアリングする場合は QR コードを使用してください

### 修正

- 別の端末で宛先を削除したとき、アプリが最小化中でも削除を反映し、画面へ戻った際にもすぐに同期するようにしました
